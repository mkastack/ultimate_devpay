...
import { contracts, conversations, fmtUSD, fmtGHS } from "@/lib/mock-data";
import { ReleaseDialog } from "@/components/dashboard/ReleaseDialog";
...
function ContractsPage() {
  ...
  return (
    <>
      <DashboardHeader title="Active Contracts" subtitle="Milestones, escrow, and payment releases — one card per contract." />
          ...
          return (
            <div key={c.id} className="rounded-2xl p-5 transition-all hover:-translate-y-0.5" style={{ background: "var(--surface)", border: "1px solid var(--color-border)" }}>
              ...
              <div className="mt-4 flex items-center gap-2">
                ...
                <Link to="/dashboard/messages" search={{ thread }} className="grid h-9 w-9 place-items-center rounded-[10px] text-[color:var(--text-secondary)] hover:text-white" style={{ border: "1px solid var(--color-border)" }} aria-label="Message">
